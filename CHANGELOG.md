...
- `OnHistoryLost.RESUME_FROM_NOW` now self-repairs the checkpoint: instead of starting from an implicit, non-durable "now" while the expired tokens linger in the checkpoint (leaving the heartbeat to probe a dead token in a loop until the next event), the recovery captures a fresh server-certified position, persists it as a startup precondition, and resumes from it. The heartbeat never consults the expired token again.
