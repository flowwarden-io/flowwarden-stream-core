...
- `lastSeenToken` now advances only on **settled** events: handler success (after the processed anchor had its chance to be written), rejection by `@Filter`, absence of a matching handler, or a terminal skip/DLQ decision. An event still being processed or retried no longer advances it — previously the token was recorded at *receipt*, so the periodic flush or an idle probe chained from it could certify past a delivery still in flight; combined with a history-lost recovery clearing the processed anchor, a crash at the wrong moment silently lost that delivery. An interrupted retry backoff (shutdown while waiting between attempts) is **not** a settled outcome: the event stays replayable and its token is never published. DLQ reserve: the terminal *decision* is what settles the event — the DLQ write itself is best-effort (failure signaled via `onEventDlqFailed`), so an event abandoned after exhaustion is settled whether or not it was durably dead-lettered, as before this release. The stronger semantics also mean `ResumeStrategy.SEEN_FIRST` can no longer skip an in-flight handler on restart.
- `saveIntervalSeconds` is now purely a write-coalescing flush: each tick persists the latest settled event token **only if it changed** since the last flush, and never opens any cursor. Keeping idle streams recoverable is the separate responsibility of `idleHeartbeatIntervalSeconds` — the two cadences are orthogonal (write pressure vs oplog protection) and independently tunable. A clean tick writes nothing; `lastSeenTimestamp` only moves when the position moves, so a frozen checkpoint document is a real signal again instead of being rewritten identically forever.
...
- `lastSeenToken` never regresses. A `ResumeStrategy.PROCESSED_FIRST` restart seeds the heartbeat with the (older) processed token, and the events replayed from it carry tokens older than the persisted seen position — both used to be written back as the "latest seen" position, destroying the level-2 safety net of the resume cascade (the replay regression predates the heartbeat and affected the previous timer as well). Resume seeds are now distinguished from delivered events, and a stream that resumes behind its persisted position enters an explicit **catch-up state**: replayed events are neither persisted as seen nor allowed to refresh the heartbeat (nothing has been re-certified), and a probe returning a server-certified empty interval ends the catch-up by establishing a fresh safe position. No client-side ordering of resume tokens is involved — the driver specification defines none.
...
- Resuming from the secondary token when the primary was never recorded (e.g. right after a history-lost recovery, before any handler success) is no longer reported as a degradation: it resumes with an INFO log instead of a misleading "aged out of oplog" warning and a `fallback` metric.
